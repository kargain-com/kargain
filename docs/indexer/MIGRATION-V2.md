...
| **SVM passport entity + UNION reads (S7c-4 · September 2026)** | ✅ Raw **`metadata_snapshot`** captured once at inline ingest; materialized **`kargain_svm_projection.passport`** (no custody columns) rebuilt from raw + snapshots; entity census routes UNION EVM + registered SVM namespaces in one SQL via [`src/lib/ponder-passport-entity.ts`](../../src/lib/ponder-passport-entity.ts). **`duplicateVin`:** EVM via Ponder handlers on `kargain.passport` only; SVM via projection post-pass on SVM rows only — **not merged across networks**. Production enable deferred to **S9** ([OPERATIONS.md §S9](./OPERATIONS.md)) |
