...
| **SVM provenance projection + UNION reads (S7c-2 · September 2026)** | ✅ Materialized **`kargain_svm_projection`** (`passport_record`, `passport_uri_history`) rebuilt from raw by **`svm-ingest`**; Hono provenance reads UNION EVM + SVM in one SQL via [`src/lib/ponder-passport-provenance.ts`](../../src/lib/ponder-passport-provenance.ts). **`passport.custodyChain` unchanged.** Production enable deferred to **S9** ([OPERATIONS.md §S9](./OPERATIONS.md)) |
...
| `RecoveredLockedHome` | — | **Not** in crossing stream (no `guid`; admin recovery path) |
