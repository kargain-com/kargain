...
| Passport browse filter indexes (August 2026) | ✅ Expression indexes on `passport`: column btree for `status`, `year`, `mileageKm`, `locationPlaceId`; `lower(col)` btree for `make`, `model`, `fuelType`, `bodyType`, `transmission`, `condition`, `vehicleType` (match browse SQL). **No index** for `colour` / `search` (`ILIKE '%…%'` — revisit at ~50k passport rows or when those filters dominate latency; no `pg_trgm` yet). **VPS full reindex required** before ship ([OPERATIONS.md](./OPERATIONS.md)); do not reindex until index set accepted |
