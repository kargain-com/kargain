...
| **SVM passport entity + UNION reads (S7c-4 · September 2026)** | ✅ Raw **`metadata_snapshot`** captured once at inline ingest (**append-only**; new document hash → new row; **`status=unavailable`** with **`content_sha256` NULL** on fetch failure — observation-keyed id; not a fifth `ingest_refusal` kind; CHECK binds digest↔status; partial unique on captured only); materialized **`kargain_svm_projection.passport`** (no custody columns) rebuilt from raw + snapshots only; entity census routes UNION EVM + registered SVM namespaces in one SQL via [`src/lib/ponder-passport-entity.ts`](../../src/lib/ponder-passport-entity.ts). **`duplicateVin`:** EVM via Ponder handlers on `kargain.passport` only; SVM via projection post-pass on SVM rows only — **not merged across networks** (UI: per-network index). Production enable deferred to **S9** ([OPERATIONS.md §S9](./OPERATIONS.md)) |
| **UNION HTTP + Ponder 0.16 physical names (S8-5-fix · September 2026)** | ✅ Product loaders **always** `UNION ALL` (empty SVM arm until svm-ingest). Apply empty [`projection-schema.sql`](../../src/svm-ingest/db/projection-schema.sql) on any Postgres that serves UNION HTTP. **Physical fact (measured):** Ponder 0.16 creates `kargain.*` columns in **snake_case** (`chain_id`, `mileage_km`, `denomination_kind`); `SELECT "chainId"` fails with column does not exist. JS / Drizzle / `PASSPORT_BROWSE_INDEXES` still use camelCase field names — that is not the PG wire. Production master browse is Drizzle (`ponder-consignment-browse.ts`); quoted camelCase raw SQL never shipped on master. Local UNION namespaces live in [`ponder-read-namespaces.ts`](../../src/lib/ponder-read-namespaces.ts), not `COMMERCIAL_ACTIVE`. |
| **EVM event disposition (S7-event-disposition · September 2026)** | ✅ All **110** commercial ABI events carry exactly one disposition kind: **78 census** + **30 out_of_scope** + **2 divergence-only** (D-38, D-39); D-40–D-42 annotate census rows in [`named-divergences.json`](../../svm/crates/kargain-events/named-divergences.json). Manual rows in [`event-dispositions.json`](../../svm/crates/kargain-events/event-dispositions.json); validated on `pnpm generate:svm-event-manifest` + `test:verify` |
...
**Custody (S7c-3):** Stored `passport.custodyChain` / `custodyUpdatedAt` **removed**. Handlers append **`custody_determining_event`** (`PassportMinted`, `PassportBridgeMinted`, `CustodyLockSet(unlock)`, origin `VerificationReset`). **`bridge_crossing`** (S7b) is stream A. Read owner [`src/lib/ponder-passport-custody.ts`](../../src/lib/ponder-passport-custody.ts) folds at HTTP time; incomplete fold → `custodyUnresolved` (never origin fallback). Cross-writer order uses **guid linkage + per-writer log order only** — not timestamps. **Entity (S7c-4):** passport denorm + browse/detail UNION via [`src/lib/ponder-passport-entity.ts`](../../src/lib/ponder-passport-entity.ts); custody stays fold-at-read only.
