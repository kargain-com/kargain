...
| KarPro on profile (RSC) | Server `isActiveVerifier` / public profile badge may remain hub-scoped until a later cutover; owner edit / widget / stats stake reads use wallet commercial target |
...
*Document version: 5.75 (July 2026 — §4.11 KarPro multi-chain wallet commercial target). Update when tokens, app shell, or component contracts change.*