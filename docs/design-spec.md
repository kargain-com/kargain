...
- **Chrome matches Bridge:** Level B card `rounded-md border border-border-default bg-bg-card p-4`, heading `font-sans text-base font-medium text-text-primary`, body `font-sans text-sm text-text-secondary`, CTAs `Button variant="secondary" className="w-full"`. Entry CTA strings live only in [`sell-copy.ts`](../lib/passport/sell-copy.ts) (**List** · **Delegate** · **Auction** — each distinct).
- Fixed price shows **List** (self open → edit page) and **Delegate** (grant) while encumbrance `may(OpenConsignment)` is available and no live consignment holds the NFT — **status-agnostic** (UNVERIFIED may list).
