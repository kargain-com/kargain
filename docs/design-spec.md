...
**Sizing:** use the `size` prop — **16** (inline), **20** (buttons, nav), **24** (decorative). Color via `className` + `currentColor` (`text-text-secondary`, `text-accent-warm`, `text-status-error`, etc.). Do not pass `strokeWidth` — the mono set is filled; seven lucide-bridge components render stroke mode internally: `BookmarkCheckIcon`, `CheckDoubleIcon`, `GlobeIcon`, `ShieldIcon`, `ShieldWarningIcon`, `ShieldCheckIcon`, `WalletIcon`.
...
*Document version: 5.28 (July 2026 — Mono Icons migration complete; §7 rewrite; `lucide-react` removed). Update when tokens, app shell, or component contracts change.*