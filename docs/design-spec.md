...
**Location PlacePicker (create + edit):** [`place-picker.tsx`](../components/geo/place-picker.tsx) replaces free-text location. Debounced city search via same-origin [`/api/geo/suggest`](../lib/geo/client.ts); **Use my location** uses `navigator.geolocation` then [`/api/geo/reverse`](../lib/geo/client.ts) (GPS stays in memory only — never form state, never Arweave). Selected value writes `placeId` + `countryCode` + `label` (+ optional `city`/`region`) into metadata wire; incomplete drafts fail with *Select a city from the list.* Spec grid and metadata diff show **label only** (fallback `city, countryCode`) — never coordinates. Legacy label-only / lat-lng metadata still parses for display; edit hydrates only a complete Place selection.
...
| Passport list | [`map-profile-passport.ts`](../lib/passport/map-profile-passport.ts) — Ponder origin `chainId` + `custodyChain`; Level C card link `/marketplace/{tokenId}?chain={custodyChain}`; `PassportIdLabel` uses origin; when `custodyChain !== chainId`, quiet mono tertiary **on {shortChainName(custodyChain)}** (bridged-away — ownership retained) |
...
*Document version: 5.81 (July 2026 — §4.10 Location PlacePicker + selection-only placeId wire). Update when tokens, app shell, or component contracts change.*