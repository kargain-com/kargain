...
| Action banner | `ProfileActionBanner` — five contextual cases (visitor+KarPro send request, owner become KarPro, owner open challenges on verifications, etc.) |
...
| Disputes panel | `serialLabel` eyebrow *Challenges on your verifications*; Level A `DisputeCard` grid — challenged-verifier awareness (not a resolve inbox) |
| Dispute cards | Vehicle make/model/year, reason, window remaining, exclusion copy (recorded verifier cannot resolve), disputer, **View passport** link — never Resolve |
