...
**"Create passport":** Secondary border style — `border border-border-hover bg-transparent`. Desktop only (`hidden md:inline-flex`); mobile uses bottom-nav center FAB. **Mint path today is EVM-only** (`requireEvmSession` + `mintPassport`). Solana `MintPassport` is **config-authority only** — product Create cannot mirror EVM “anyone mints” without a program change. On a Solana commercial session the control must name that absence (§4.7), never silently offer an Ethereum switch as if mint were dual-VM permissionless.
