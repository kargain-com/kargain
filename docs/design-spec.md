...
- Unframed group headed **Sell** (`space-y-4`, no Level B border/surface shell), ordered quiet mode lines **Fixed price** → **Auction** (user-facing; not Ascending). Entry CTA strings live only in [`sell-copy.ts`](../lib/passport/sell-copy.ts).
- Fixed price shows unique CTAs **List** (self open → edit page; `cursor-default` on hover) and **Delegate** (grant) while encumbrance `may(OpenConsignment)` is available and no live consignment holds the NFT — **status-agnostic** (UNVERIFIED may list). Every Sell entry button has a distinct label — never two identical CTA words.
- Auction **direct open** requires an active KarPro owner **and** `passportStatus === VERIFIED` (mirrors chain `PassportNotVerified`); create-panel submit remains **Start auction**. Auction **grant** CTA is **Auction** (private owners only, status-free) so an agent may verify then open — mutually exclusive with direct open by verifier status. When a KarPro owner is blocked solely by non-VERIFIED status, show quiet [`AUCTION_REQUIRES_VERIFICATION_HINT`](../lib/auction/sale-form-copy.ts). Runner note for private owners: [`SELL_AUCTION_RUNNER_NOTE`](../lib/passport/sell-copy.ts).
