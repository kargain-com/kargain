...
**"Create passport":** Secondary border style — `border border-border-hover bg-transparent`. Desktop only (`hidden md:inline-flex`); mobile uses bottom-nav center FAB. Product Create admits via [`create-passport-surface`](../lib/passport/create-passport-surface.ts): `surfaceSupport("create_passport", namespace)` **before** session narrowing. When the namespace does not admit creation (SVM `authority_only`), the page shows that census sentence and names where creation is available — never the Ethereum-wallet sentence. Wallet-family refusal (`EvmSessionRefusal`) appears only on namespaces that admit creation. Mint body remains EVM `mintPassport`. Solana `MintPassport` stays config-authority only; nothing promises a self-service mint later.
...
**Chain selector:** [`ChainSelector`](../components/shell/chain-selector.tsx) — Radix dropdown, full network name. Admission is by **commercial namespace** (EVM or SVM row in `COMMERCIAL_ACTIVE`), not by the wagmi write-union alone. The picker lists every registered commercial network. Wrong-network only when the wallet is on the same virtual machine but a different commercial network, or (when URL `?chain=` is set) mismatches that namespace — never because a hub default is unset. Same-VM switch labels use the commercial network label; cross-VM selection never calls a chain switch. Desktop only (`hidden md:flex`).
...
**Wallet:** [`WalletLoginButton`](../components/wallet-login-button.tsx) — identicon + ENS or short address + ChevronDown. Radix dropdown: View on explorer, Copy address, Disconnect. Disconnected: opens connect dialog with **WalletConnect** (when `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID` is set), **Browser wallet** (injected extension or wallet in-app browser), and **Solana wallets** discovered via Wallet Standard (lazy on dialog open, same registration shape as WalletConnect). Mobile Safari/Chrome without an injected provider: hint to use WalletConnect or open the site in MetaMask/Coinbase Wallet; empty state when no connector or Solana wallet is available. Dialog closes on successful connect only. One active session: connecting Solana clears the EVM session and vice versa.
...
**Network scope** ([`lib/commerce/browse-source.ts`](../lib/commerce/browse-source.ts)): URL `?chain=` is admitted before fetch. Unscoped (no chain) keeps today's multi-network consignments browse. A selected EIP-155 commercial namespace filters `chainId` on `GET /consignments`. A selected commercial namespace with no indexed commerce source (Solana today — projection has no consignments) refuses with `no_indexed_commerce` and the owner's sentence — never another network's lots, never the empty-filter grid copy (*No active listings match these filters yet.*). Same admit owns `/auctions` ([`searchActiveAuctions`](../app/actions/auction-browse.ts)); empty auctions keep *No active auctions*. Homepage auction count stays unscoped.
...
| KarPro hub (`/kar-pro`) | Target chain = connected wallet when commercial ([`resolveKarProTargetChainId`](../lib/kar-pro/kar-pro-target-chain.ts)); non-commercial → switch prompt ([`KarProNetworkPrompt`](../components/kar-pro/kar-pro-network-prompt.tsx)). Join / stake / leave / fee / profile writes use `karProStakingAddress`/`karProPassAddress(chainId)` + wagmi `chainId`. Join is **native stake only** (`becomeVerifierNative`). Nuclear #7 has no token-join path; historical Nuclear #4 bytecode exposed a dormant token path that was never enabled — no token-join UI either way. Multi-chain truth: [`membership-roster.ts`](../lib/kar-pro/membership-roster.ts) + [`use-kar-pro-membership-roster.ts`](../hooks/use-kar-pro-membership-roster.ts) keyed `isActiveVerifier` per `COMMERCIAL_ACTIVE`; [`KarProNetworksRoster`](../components/kar-pro/kar-pro-networks-roster.tsx) on join path and Overview. Identity strip: mono **Network · {shortChainName}** + tabular id; Pass ID **`showChain={false}`** (network line owns the name). Join: **Creating on…** + per-network disclosure + *Already KarPro on…* when active elsewhere. |
...
| Key rotation | **Not a task.** Do not open prompts or queue items on rotation / epoch / “identity continuation vs change.” Revisit only if a **demonstrated** hole shows the current model is broken (then change the model deliberately) |
...
**Encumbrance permission** ([`lib/passport/encumbrance-permission.ts`](../lib/passport/encumbrance-permission.ts)): sole answer to `may(OpenConsignment)` / `may(LeaveChain)`. Outcomes are available, or blocked with named causes `refused` · `source_unanswerable` · `reads_unresolved` · `fee_payer_required` · `construction` · `simulation_unavailable` · `unmapped_program_error` · `not_in_program` · `product_owner_owed` · `authority_only`. `source_unanswerable` may carry a known source address (EVM) or `not_carried_by_vm` (SVM Custom(20) — never invent an address). Decoded from keyed multicall / May simulate, never message strings. Unresolved is waiting copy — never presented as a definite refusal. Sell and bridge consume this gate and do not invent permission copy. Refusal chrome follows §4.21 (wait ≠ refusal; no empty region for a blocked gate).

**Encumbrance registry** ([`lib/passport/encumbrance-registry.ts`](../lib/passport/encumbrance-registry.ts) + [`passport-encumbrance-registry.tsx`](../components/passport/passport-encumbrance-registry.tsx)): Level B factual readout of sources registered on this custody-chain KarPassport (`encumbranceSourceCount` + `encumbranceSourceAt`, cap 8). Chain-scoped — never assume one chain’s registry for another. When a permission refusal names a registered source, that row gets a quiet *Could not answer* line. A refused registry fact renders [`commerceFactCauseCopy`](../lib/passport/commerce-fact.ts) (§4.21) — never an empty section.
...
| Rule | Value |
|------|-------|
| Seller list UI | [`listing-seller-settlement-panel.tsx`](../components/marketplace/listing-seller-settlement-panel.tsx) + [`listing-edit-client.tsx`](../components/marketplace/listing-edit-client.tsx); FixedPrice **open** is dual-VM via [`open-fixed-price-consignment.ts`](../lib/commerce/open-fixed-price-consignment.ts) (EVM `openDirect`; SVM native OpenDirect only — fiat/SPL/settlement note refuse by name). Settlement asset labels from [`settlement-asset-meta.ts`](../lib/commerce/settlement-asset-meta.ts) (ETH / USDC / short address); Asset/Fiat denomination from [`deriveOpenableTerms`](../lib/commerce/openable-terms.ts) (`fixedPrice`) via Ponder `GET /commerce-payment-tokens` + `/commerce-currency-feeds` (never a static per-chain fiat map). Asking price input shows the unit (`USDC` / `ETH` / fiat code). Fiat is withheld with *Fiat pricing needs a measured price feed, which this network does not have* when the selected settlement asset has no measured feed; Asset denomination remains selectable. Mode unset → *Fixed-price selling is not available on this chain yet.* Config unread → *Settlement terms for this chain are not available yet.* (fail closed). |
...
- [`sell-surface.ts`](../lib/passport/sell-surface.ts) is the pure visibility policy. Successful inactive chain listing state is required for the group; active listing, live auction, settlement hold, or non-owner returns all flags false / self-open `null` and carries `closedCause`. Open/grant flags require `openConsignmentPermission` available from [`encumbrance-permission.ts`](../lib/passport/encumbrance-permission.ts); refused / unanswerable / unresolved all hide CTAs. When no CTA is visible, the panel always renders a named sentence: blocked gate → [`encumbrancePermissionCopy`](../lib/passport/encumbrance-permission.ts); otherwise carried `closedCause` → [`sellSurfaceClosedCopy`](../lib/passport/sell-surface.ts) (fact causes via [`commerceFactCauseCopy`](../lib/passport/commerce-fact.ts)). Refusal chrome follows §4.21 — no silent empty region for a carried cause.
...
| Visibility | Pure [`deriveBridgeSurface`](../lib/passport/bridge-surface.ts) — owner only. Star membership via [`admitBridgeCrossingRoute`](../lib/web3/bridge/bridge-config.ts) (`EID_BY_CHAIN`); absent → `crossingRoute.absent` + sole sentence [`bridgeCrossingRouteCauseCopy`](../lib/web3/bridge/bridge-config.ts) (`no_crossing_route`) — never silent hide for an owner, never folded into leave-permission `blockReason`. When configured: leave permission from [`encumbrance-permission.ts`](../lib/passport/encumbrance-permission.ts) (`leaveChainPermission` gate); leave-permission unread fail-closed as `blockReason: "unresolved"` (`reads_unresolved` wording); fold gaps and lock-unread are a separate `location` / `locationCopy` answer from the same owner via [`derivePassportPresence`](../lib/passport/presence.ts) (§4.21) — never reported as leave-permission unread; `source_unanswerable` may be known address or `not_carried_by_vm`; plain `refused` may refine to consigned / challenged when those facts are known; disabledReason always comes from [`bridgeBlockReasonCopy`](../lib/passport/bridge-surface.ts) → permission copy (§4.21 — no empty region) |
...
| Registry | Chain-read membership on this custody passport ([`encumbrance-registry.ts`](../lib/passport/encumbrance-registry.ts)); mono tabular addresses; highlight a registered source named by `source_unanswerable`; refused registry → [`commerceFactCauseCopy`](../lib/passport/commerce-fact.ts) (§4.21) |
...
**Commerce refusal chrome.** The same wait ≠ refusal / no-empty rule applies to encumbrance permission gates and refused commerce facts: blocked gates use [`encumbrancePermissionCopy`](../lib/passport/encumbrance-permission.ts); refused facts use [`commerceFactCauseCopy`](../lib/passport/commerce-fact.ts); sell closed-causes use [`sellSurfaceClosedCopy`](../lib/passport/sell-surface.ts). Waiting sentences (`reads_unresolved`, `live_pending`, pending registry) never equal a definite refusal sentence.
...
- [x] Design-spec checklist closed; ship recorded in the maintainer milestone log after ship
