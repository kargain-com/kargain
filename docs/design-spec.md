...
| Buy panel | Hero via [`listing-display-price.tsx`](../components/marketplace/listing-display-price.tsx) from [`listing-price-display.ts`](../lib/commerce/listing-price-display.ts): **Asset** → token units + settlement label (same units as `quoteBuy`); **Fiat** → `convertPrice()`; settlement asset fixed at open (native or admitted ERC-20 via `buy(tokenId)`) |
