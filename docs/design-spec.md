...
| Agent dashboard | **Consigned vehicles** tab on `/profile/[handle]` when owner + active KarPro ([`consigned-vehicles-tab.tsx`](../components/profile/consigned-vehicles-tab.tsx)); awaiting section via Ponder `GET /agents/:address/authorizations?hasActiveListing=false` (paginated) + `/passports/batch` enrichment; chain `agentAuthorizations` filters stale rows; active/past sections via `GET /agents/:address/listings`. **Active consignments** on [`/pro/[slug]`](../app/pro/[slug]/page.tsx) with **View all N consignments →** to `?tab=consigned` when showroom page truncates at 100. Display only — `listOnBehalf` / `agentDelist` deferred |
