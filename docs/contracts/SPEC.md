...
| D-30 | Payment-token admit proves mint account via money-crate `require_admitted_spl_mint_account` (owner Token\|Token-2022; classic 82-byte OK; Token-2022 TLV at offset 166 after AccountType@165; TransferFee → `TransferFeeExtensionForbidden`; decimals from mint byte 44 — never ix args). FoT closed at admit. FixedPrice buy does **not** carry `ShortDelivery` / `require_full_delivery` (unreachable after honest admit; classic Tokenkeg cannot under-deliver). Soft-revoke does not re-check `enabled` on buy. EVM retains ShortDelivery (Erc20Admission does not ban FoT) — named asymmetry. | ERC-20 `balanceOf` delta + no FoT ban at admit | Proven mint properties; no unreachable buy-time ShortDelivery guard |
