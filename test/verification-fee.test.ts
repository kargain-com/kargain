...
describe("formatVerificationFee", () => {
  it("returns contact copy for zero fee", () => {
    assert.equal(formatVerificationFee(0n, ethUnit), "Contact for quote");
  });

  it("formats positive wei as ETH", () => {
    assert.equal(formatVerificationFee(FEE_005_ETH, ethUnit), "0.05 ETH");
  });
});

describe("verificationFeeToUsd1e8", () => {
  it("returns zero for non-positive fee or rate", () => {
    assert.equal(verificationFeeToUsd1e8(0n, ETH_USD_3000, ethUnit), 0n);
    assert.equal(verificationFeeToUsd1e8(FEE_005_ETH, 0n, ethUnit), 0n);
    assert.equal(verificationFeeToUsd1e8(-1n, ETH_USD_3000, ethUnit), 0n);
  });

  it("converts 0.05 ETH at $3000 to USD 1e8", () => {
    assert.equal(
      verificationFeeToUsd1e8(FEE_005_ETH, ETH_USD_3000, ethUnit),
      150_000_000_00n,
    );
  });
  ...
  it("rounds down on remainder", () => {
    ...
    const usd = verificationFeeToUsd1e8(feeWei, ethUsd, ethUnit);
...
describe("verificationFeeInUsdc", () => {
  it("returns zero when fee is zero", () => {
    assert.equal(verificationFeeInUsdc(0n, ETH_USD_3000, ethUnit), 0n);
  });

  it("returns zero when ethUsd rate is zero", () => {
    assert.equal(verificationFeeInUsdc(FEE_005_ETH, 0n, ethUnit), 0n);
  });

  it("converts wei fee to USDC 1e6 using ethUsd at 1e8", () => {
    assert.equal(
      verificationFeeInUsdc(FEE_005_ETH, ETH_USD_3000, ethUnit),
      150_000_000n,
    );
  });
});
