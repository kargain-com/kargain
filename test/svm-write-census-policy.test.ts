...
const ENUMERATED_WRITE_FILES: readonly string[] = [
  ...
  "hooks/use-withdraw-challenge.ts",
...
describe("svm write census policy", () => {
  it("enumerated write files match detected product write-site files (both directions)", () => {
    ...
    assert.ok(
      detected.includes("hooks/use-append-passport-record.ts"),
      "append-passport-record port hook must be in the write-file set",
    );
    assert.ok(
      detected.includes("hooks/use-report-passport-discrepancy.ts"),
      "report-passport-discrepancy port hook must be in the write-file set",
    );
    assert.ok(
      detected.includes("hooks/use-append-passport-attestation.ts"),
      "append-passport-attestation port hook must be in the write-file set",
    );
    assert.ok(
    ...
    assert.ok(
      detected.includes("hooks/use-withdraw-challenge.ts"),
      "withdraw-challenge port hook must be in the write-file set",
    );
    assert.ok(
      detected.includes("hooks/use-judge-challenge.ts"),
      "judge-challenge port hook must be in the write-file set",
    );
    assert.ok(
      detected.includes("hooks/use-conclude-challenge.ts"),
      "conclude-challenge port hook must be in the write-file set",
    );
    assert.ok(
      detected.includes("hooks/use-set-verification-fee.ts"),
      "set-verification-fee port hook must be in the write-file set",
    );
  });
