...
import { ponderFetch } from "@/lib/web3/ponder-fetch";
 ...
 * from the Ponder HTTP API (tagged `"use cache"`) plus full metadata JSON per
 * tokenUri. Derivation runs client-side in the KarPro Commons section.
 *
...
export async function getCommonsObservations(): Promise<CommonsObservationsResult> {
  return fetchVerifiedObservations({
    ...
    fetchPonderJson: async (url) => {
      const res = await ponderFetch("passports", url);
      if (!res.ok) {
